        ...
        <>
            {wishListItems.length === 0 ? (
                <div className="my-5">
                    ...
                    <h2 className="my-5 text-center text-green-600">Wish List is Empty</h2>
                </div>
            ) : (
                <>
                    ...
                    {wishListItems.map((item) => (
                        <div key={item.id} className="flex itmes-center bg-gray-100 mb-4 rounded-2xl p-4">
                            <div className="w-52 h-52 mr-4">
                                <img className="w-full h-full object-cover rounded-lg" src={item.imageCover} alt="product image" />
                            </div>
                            <div className="grow flex items-center">
                                <div className="flex flex-col gap-3">
                                    <h2 className="text-2xl">{item.title}</h2>
                                    <p>Price: {item.price}EGP</p>
                                    <button onClick={() => { handleRemoveItemFromWishList(item.id) }} className="w-60 px-4 py-2 rounded-lg flex gap-3 justify-center items-center bg-red-600 text-white">
                                        <FaTrash />
                                        Remove
                                    </button>
                                </div>
