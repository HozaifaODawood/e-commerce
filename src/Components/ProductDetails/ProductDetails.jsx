...
export default function ProductDetails() {
    ...
    const getProductDetails = () => axios.get(`https://ecommerce.routemisr.com/api/v1/products/${pId}`);
        ...
        <>
            ...
            <div className="grid grid-cols-12 gap-4 mt-7">
                <div className="col-span-12 md:col-span-4">
                    <img src={data.data.data?.imageCover} className="w-full max-w-60 md:max-w-full mx-auto " alt="" />
                </div>
                <div className="col-span-12 md:col-span-8 self-center">
                    ...
                    <div className="flex justify-between mb-2">
                        ...
                        <span className="flex items-center gap-2" >{data.data.data?.ratingsAverage} <FaStar className="text-yellow-500" /> </span>
                    </div>
                    <div className="cart-wish flex flex-wrap justify-around items-center px-6">
                        <button disabled={isLoading} onClick={()=>handleAddProductToCart(data.data.data?.id)} className="btn-green w-8/12 mt-2">
                            {
                                isLoading ?
                                'Loading ...' :
                                'Add Product'
                        ...
                        <button onClick={()=>handleAddProductToWishList(data.data.data?.id)}><FaHeart className={`inline-block text-3xl text-${wishColor}`} /></button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5">
                {
                    relatedProducts.data.data.filter(p => p.category._id === cId).slice(0, 5).map((p) => <ProductCard product={p} key={p.id} />)
                }
            </div>
