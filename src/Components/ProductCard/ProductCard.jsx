        ...
        <>
            ...
            <div className="overflow-hidden group bg-white rounded p-3 hover:shadow-md hover:shadow-green-600">
                <Link to={`/productdetails/${product?.id}/${product?.category._id}`}>
                    <img className="w-full md:h-52 object-cover object-center" src={product?.imageCover} alt="" />
                    <span className="text-green-600">{product?.category.name}</span>
                    <h2 className="text-base font-semibold mb-3">{product?.title.split(" ", 2).join(" ")}</h2>
                ...
                <div className="cart-wish flex justify-between items-center">
                    <button disabled={isLoading} onClick={()=>handleAddProductToCart(product?.id)} className="btn-green w-full mt-2 translate-y-28 group-hover:translate-y-0 transition-all duration-500">
                        {
                            isLoading ? 
                            'Loading ...':
                            'Add Product'
                    ...
                    <button onClick={()=>handleAddProductToWishList(product?.id)}><FaHeart className={`text-2xl text-${wishColor}`} /></button>
                </div>
